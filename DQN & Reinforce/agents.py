    ...
    **YOU NEED TO IMPLEMENT FUNCTIONS IN THIS CLASS**
    ...
    def schedule_hyperparameters(self, timestep: int, max_timestep: int):
        ...
        ** You may modify the signature of these functions if you wish to pass additional arguments **
        ...
        def epsilon_linear_decay(*args, **kwargs):
            ### PUT YOUR CODE HERE ###
            epsilon = self.epsilon_start - (self.epsilon_start - self.epsilon_min) * min(timestep / (self.exploration_fraction * max_timestep), 1)
            return epsilon
        ...
        def epsilon_exponential_decay(*args, **kwargs):
            ...
            if timestep >= max_timestep:
            	...
            	decay_factor = np.power(self.epsilon_exponential_decay_factor, 1.0 / max_timestep)
            	epsilon = decay_factor ** timestep * self.epsilon_start
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        return torch.argmax(self.critics_net(obs_torch)).numpy()
    ...
    def update(self, batch: Transition) -> Dict[str, float]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        ### PUT YOUR CODE HERE ###
        state = batch.states
        action = batch.actions.type(torch.LongTensor)
        reward = batch.rewards
        next_states = batch.next_states
        done = batch.done
        
        predicted_q = self.critics_net(state).gather(1, action)
        best_q = torch.max(self.critics_target(next_states), 1)[0].detach()
        expected_q = (1 - done.squeeze(1)) * best_q * self.gamma + reward.squeeze(1)
        
        q_loss = torch.nn.functional.mse_loss(predicted_q.squeeze(1), expected_q)
    ...
    ** YOU NEED TO IMPLEMENT THE FUNCTIONS IN THIS CLASS **
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        if explore == True:
            act_prob = self.policy(obs_torch).detach().numpy()
    ...
    def update(
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        for log_prob, G in zip(log_probs, discounted_rewards):
            p_loss.append(-log_prob * G)  
        p_loss = torch.stack(p_loss).sum()
