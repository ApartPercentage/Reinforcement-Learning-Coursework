...
class ValueIteration(MDPSolver):
        ...
        **DO NOT ALTER THE MDP HERE**
        ...
        while (True):
            ...
            for s in np.arange(self.state_dim):
            	v = V[s]
            	vect = self.mdp.P[s,:,:] * (self.mdp.R[s,:,:] + self.gamma * V)
            	V[s] = np.max(np.sum(vect, axis = 1))
    ...
    def _calc_policy(self, V: np.ndarray) -> np.ndarray:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        for s in np.arange(self.state_dim):
        	vect = self.mdp.P[s,:,:] * (self.mdp.R[s,:,:] + self.gamma * V)
        	best = np.argmax(np.sum(vect, axis = 1))
        	policy[s, best] = 1.0
            ...
            for state in self.mdp.states:
                ...
                for action in self.mdp.actions:
                    for nextState in self.mdp.states:
                        ...
                        * (self.mdp.R[self.mdp._state_dict[state], self.mdp._action_dict[action], self.mdp._state_dict[nextState]] + self.gamma * V[self.mdp._state_dict[nextState]]))
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        while (True):
            ...
            for state in self.mdp.states:
                ...
                for action in self.mdp.actions:
                    for nextState in self.mdp.states:
                        returns[self.mdp._action_dict[action]] += self.mdp.P[self.mdp._state_dict[state], self.mdp._action_dict[action], self.mdp._state_dict[nextState]] * (self.mdp.R[self.mdp._state_dict[state], self.mdp._action_dict[action], self.mdp._state_dict[nextState]] + self.gamma * V[self.mdp._state_dict[nextState]])
